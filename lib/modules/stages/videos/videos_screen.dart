...
import 'package:mal/modules/stages/cubit/stages_states.dart';
import 'package:mal/modules/stages/sub_stages.dart';
import 'package:mal/modules/stages/videos/question_item.dart';
import 'package:mal/modules/stages/videos/submit_button.dart';
import 'package:mal/modules/stages/videos/video_widget.dart';
...
class VideosScreen extends StatefulWidget {
  ...
  final StageDetails stagesDetails;

  const VideosScreen({
    Key? key,
    required this.stagesDetails,
...
class _VideosScreenState extends State<VideosScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar:
          CustomAppBar(title: LocaleKeys.stage.tr() + '  ${widget.stageIndex}'),
      body: BlocConsumer<StagesCubit, StagesStates>(
        listener: (context, state) {
          if (state is AnswerAllQuestion) {
            CustomSnackBar.buildErrorSnackBar(context, state.message);
          }
          /*else if (state is SubmitAnswersSuccessState) {
            CustomAlert.buildAlert(
                context, buildAlertContent(message: state.message, status: 1));
          }*/
          else if (state is StagePassedState) {
            CustomAlert.buildAlertCanNotPop(
                context, buildAlertContent(message: state.message, status: 1));
          } else if (state is StageFailedState) {
            CustomAlert.buildAlert(
                context, buildAlertContent(message: state.message, status: 0));
          } else if (state is SubmitErrorConnectionState) {
            CustomSnackBar.buildErrorSnackBar(context, state.error);
            CustomAlert.buildAlert(
                context, buildAlertContent(message: LocaleKeys.ops, status: 3));
          } else if (state is SubmitAnswersErrorState) {
            CustomSnackBar.buildErrorSnackBar(context, state.error);
          } else if (state is GetStageDetailsErrorState) {
            CustomSnackBar.buildErrorSnackBar(context, state.error);
          }
        },
        builder: (context, state) {
          ...
          if (cubit.getStageDetailsLoading) {
            return cubit.getStagesErrorConnection
                ? ErrorNetworkConnection(
                    onCallback: () {
                      cubit.getStageDetails(
                          int.parse(widget.stagesDetails.id.toString()));
                    },
                  )
                : Center(
            ...
            return Container(
              ...
              child: ListView.builder(
                  ...
                  itemCount: widget.stagesDetails.questions!.isEmpty
                      ? 1
                      : widget.stagesDetails.questions!.length,
                  itemBuilder: (_, index) {
                    return Column(
                      children: [
                        Visibility(
                        ...
                        widget.stagesDetails.questions!.isNotEmpty?BuildQuestionItem(
                          question: widget.stagesDetails.questions![index],
                          index: index + 1,
                          cubit: cubit,
                        ):Container(),
                        Visibility(
                          visible: widget.stagesDetails.questions!.isEmpty||widget.stagesDetails.questions!.length - 1 == index,
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.end,
                            children: [
                              Text(widget.stagesDetails.coachName??''),
                              SizedBox(
                                width: 70.w,
                              ),
                              SubmitButton(
                                onTap: () {
                                  cubit.submit(widget.stagesDetails
                                      .questions![index].stageId as int);
                                },
                                submitLoading: cubit.submitLoading,
                              ),
                            ],
                          ),
                        )
